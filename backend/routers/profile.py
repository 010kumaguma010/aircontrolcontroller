...
def _get_or_404(db: Session) -> ProfileRecord:
    record = db.query(ProfileRecord).filter_by(id=1).first()
    if not record:
        raise HTTPException(status_code=404, detail="プロファイルが見つかりません。サーバーを再起動してください。")
    return record


@router.get("/profile", response_model=ProfileResponse)
def get_profile(db: Session = Depends(get_db)):
    return _to_response(_get_or_404(db))


@router.post("/profile", response_model=ProfileResponse)
def update_profile(body: ProfileUpdate, db: Session = Depends(get_db)):
    record = _get_or_404(db)
